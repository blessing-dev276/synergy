...
// Live search-as-you-type sponsor picker, shared by the signup form (anon,
// pre-auth — search_sponsors is deliberately anon-callable, see
// supabase/migrations/0019_sponsor_functions.sql) and admin's sponsor
// reassignment panel (authenticated). Controlled: the parent owns
// `value = { selected: {id, display_name} | null, claimedName: string }` —
// at most one of the two is ever set at a time.
export default function SponsorPicker({ value, onChange, initialQuery = "", placeholder = "Search by name…" }) {
  const [query, setQuery] = useState(initialQuery);
  const debouncedQuery = useDebouncedValue(query, 300);
  const trimmed = debouncedQuery.trim();

  const { loading, data: results } = useSupabaseQuery(
    () => trimmed.length >= 2 && supabase.rpc("search_sponsors", { p_query: trimmed }),
    [trimmed],
  );
