...
export default function AssignmentList() {
  ...
  const { loading, error, data: assignments } = useSupabaseQuery(
    () => supabase.from("assignments").select("*").eq("published", true),
    [],
  );

  const { data: submissions } = useSupabaseQuery(
    () => user && supabase.from("assignment_submissions").select("*").eq("uid", user.id),
    [user?.id],
  );
