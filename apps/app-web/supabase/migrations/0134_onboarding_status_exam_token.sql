-- ================= fix: get_onboarding_status needs the exam's take-link token =================
-- 0133's version returned examId for a quiz-gated Level 1 lesson but not
-- the exam's public_token -- the frontend needs that to link to /take/:token
-- (same field name/shape get_my_task_flow's exam-type steps already
-- return, 0121). CREATE OR REPLACE, 0133's body plus that one lookup --
-- never edit the old migration file.
